	...
	public:
		...
		String ^FileN1,
		 ...
		 const char *FileNameIn1,
					*BufferChar1;
		 ThreadWithState^ tstat; // объявление объекта класса ThreadWithState
		 Thread				thred;// объявление объекта класса Thread
...
#pragma endregion
	public: void DoWork(int tNN, double* tXT, double* tYT, double** tZT)
	{
		...
		{
			PZT[I2][J2] = 0.5 * (cos(2 * pow((PXT[I2] / 3), 3)) - log10(sin(PXT[I2])) + 1);
		...
		// Копирование строки FileN1 в FileNameIn1 в формате ANSI
			FileStreamOut1.open(FileNameIn1, std::ios::out);
		// Открытие потока FileStreamOut1 для вывода в файл
			...
			// Формирование строки со значениями функции tZT
			...
			// Создание массива символов BufferChar1
			BufferChar1 =(char*)(void*)Marshal::StringToHGlobalAnsi(LineS1);
			// Занесение содержимого строки LineS1 в массив символов BufferChar1
			FileStreamOut1.write(BufferChar1, LineS1->Length);
			// Вывод массива символов BufferChar1 в поток
			Marshal::FreeHGlobal(IntPtr((void*)BufferChar1));
			// Освобождение памяти, выделенной для преобразования строки
			} // Конец цикла for (I2 = 1; I2 <= tNN - 1; I2++)
...
private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		// Формирование шага значения полосы прогресса
			...
			for (I1 = 0; I1 <= NN - 1; I1++) // Формированиемассива аргументов
			{
			...
			PZT[I1] = new double[NN]; // Выделение памяти для массивов элементов строк значений функции
		...
		for (I1 = 0; I1 <= NN - 1; I1++) // Цикл для освобождения памяти
		{
		delete[]PZT[I1]; // Освобождение памяти для массивов элементов значений функции
		}
		delete[]PZT; // Освобождение памяти для указателей
